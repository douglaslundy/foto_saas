...
// GET /api/essay-reviews/[id]/download-final — o cliente baixa a entrega final
// (fotos tratadas pelo fotógrafo) depois que o ensaio é marcado como entregue.
export async function GET(_request: NextRequest, { params }: Params) {
  ...
  const { data: review } = await (admin as any)
    .from('essay_reviews')
    .select('id, event_id, client_id, status, selected_photo_ids')
    .eq('id', id)
    .single() as { data: {
      id: string; event_id: string; client_id: string
  ...
  if (review.status !== 'delivered') {
    return NextResponse.json({ error: 'Suas fotos ainda não foram entregues.' }, { status: 409 })
  }
  ...
  for (let i = 0; i < photos.length; i++) {
    ...
    if (downloadError || !data) {
      console.error('[essay-reviews/download-final] storage download error:', photo.id, downloadError)
      continue
    }
  ...
  return new NextResponse(new Uint8Array(zipBuffer), {
    ...
    headers: {
      ...
      'Content-Disposition': `attachment; filename="fotos-ensaio.zip"`,
